...
public class BPSpoofIbuf implements GlobalConst  {
   ...
   *@param bpSize the basicpattern size
   *@param fd the reference to an Heapfile
   *@param Nbasicpatterns the basicpattern numbers of the page
  ...
  public  void init(
	   ...
	   Exception
    {
      ...
      n_basicpatterns = Nbasicpatterns;
   ...
   /** 
   *get a basicpattern from current buffer,pass reference buf to this method
   *usage:temp_bp = tuple.Get(buf); 
  ...
  public  BasicPattern Get(BasicPattern  buf)throws IOException, Exception
    {
      if (tot_t_proc == n_basicpatterns) done = true;
      ...
      if (t_proc == t_in_buf)
	{
      ...
      if (t_in_buf == 0)                        // No basicpatterns read in?
	{
  ...
  /**
   *
   *@return the numbers of basicpatterns in the buffer
   *@exception IOException some I/O fault
