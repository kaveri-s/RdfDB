...
public class BPSort extends BPIterator implements GlobalConst
{
  ...
  private BPOBuf            o_buf;
  private BPSpoofIbuf[]     i_buf;
	   ...
	   Exception
  {
    ...
    i_buf = new BPSpoofIbuf[n_R_runs];   // need io_bufs.java
    for (int j=0; j<n_R_runs; j++) i_buf[j] = new BPSpoofIbuf();
	...
	*/
	...
	try {
	  ...
	  throw new BPSortException(e, "Sort.java: BPUtilsException caught from Q.enq()");
	}
    ...
    while ((p_elems_curr_Q + p_elems_other_Q) < max_elems)
    {
      try
      {
        ...
        if(init_flag==1)
        {
          ...
          AttrType[] in = new AttrType[basicPattern.noOfFlds()];
          int j = 0;
          for(j = 0 ; j < (basicPattern.noOfFlds()-1)  ; j++)
          {
          ...
          s_sizes[0] = (short)((basicPattern.noOfFlds()-1) * 4 + 1* 8);
      
        if (_sort_fld==-1)
        {
          _sort_fld = basicPattern.noOfFlds();
        ...
        sort_init(in,basicPattern.noOfFlds(),s_sizes);
        init_flag = 0;
        }
      }
      catch (Exception e)
      ...
      cur_node = new BPpnode();
      cur_node.basicPattern = new BasicPattern(basicPattern); // tuple copy needed --  Bingjie 4/29/98 
  ...
  else 
  {
	...
	BPUtils.SetValue(lastElem, cur_node.basicPattern, _sort_fld);
   ...
   * @exception BPSortException something went wrong in the lower layer. 
   */
  public void close() throws BPSortException, IOException
  {
    // clean up
    if (!closeFlag) {
