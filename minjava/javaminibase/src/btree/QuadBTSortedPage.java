...
public class QuadBTSortedPage  extends THFPage{
   ...
   */
  public QuadBTSortedPage(PageId pageno, int keyType) 
   ...
   *               AttrType.attrString or AttrType.attrInteger. 
   */
  public QuadBTSortedPage(Page page, int keyType) {
   ...
   */ 
  public QuadBTSortedPage(int keyType) 
    throws ConstructPageException
    {
      super();
      try{
   ...
   */
          ...
          throws InsertRecException 
   {
     ...
     try {
       ...
       record= QuadBT.getBytesFromEntry(entry);  
       qid=super.insertRecord(record);
         if (qid==null) return null;
	 
         if ( entry.data instanceof QuadLeafData )
	   nType= NodeType.LEAF;
	 ...
	 for (i=getSlotCnt()-1; i > 0; i--) 
	   {
	     ...
	     key_i=QuadBT.getEntryFromBytes(getpage(), getSlotOffset(i), 
					getSlotLength(i), keyType, nType).key;
	     
	     key_iplus1=QuadBT.getEntryFromBytes(getpage(), getSlotOffset(i-1), 
					     getSlotLength(i-1), keyType, nType).key;
	     
	     if (QuadBT.keyCompare(key_i, key_iplus1) < 0)
	       {
	       // switch slots:
		 int ln, off;
		 ln= getSlotLength(i);
