 ...
 * abstract base class QuadIndexFile.
 * It provides an insert/delete interface.
 */
public class QuadBTreeFile extends QuadIndexFile 
  ...
  private QuadBTreeHeaderPage headerPage;
   ...
   *          of this btree file.
   */
  public QuadBTreeHeaderPage getHeaderPage() {
    return headerPage;
  }
   ...
   */
  public QuadBTreeFile(String filename)
    throws GetFileEntryException,  
	   PinPageException, 
    ...
    {      
      ...
      headerPage= new  QuadBTreeHeaderPage( headerPageId);       
   ...
   */
  public QuadBTreeFile(String filename, int keytype,
	...
	{
	  headerPage= new QuadBTreeHeaderPage(); 
  ...
  private void  _destroyFile(PageId pageno) 
	   ...
	   FreePageException
    {
      ...
      QuadBTSortedPage sortedPage;
      Page page=pinPage(pageno) ;
      sortedPage= new QuadBTSortedPage( page, headerPage.get_keyType());
      
      if (sortedPage.getType() == NodeType.INDEX) {
	QuadBTIndexPage indexPage= new QuadBTIndexPage( page, headerPage.get_keyType());
	QID      qid=new QID();
	PageId       childId;
	KeyDataEntry entry;
  ...
  public void insert(KeyClass key, QID qid) 
	   ...
	   
    {
      ...
      if (QuadBT.getKeyLength(key) > headerPage.get_maxKeySize())
	...
	QuadBTLeafPage newRootPage;
	QID dummyqid;
	
	newRootPage=new QuadBTLeafPage( headerPage.get_keyType());
      ...
      if (newRootEntry != null)
	{
	  ...
	  newRootPage = new QuadBTIndexPage(headerPage.get_keyType());
	  newRootPageId=newRootPage.getCurPage();
  ...
  private KeyDataEntry  _insert(KeyClass key, QID qid,  
	    ...
	    
    {
      ...
      QuadBTSortedPage currentPage;
      Page page;
      KeyDataEntry upEntry;
      
      
      page=pinPage(currentPageId);
      currentPage=new QuadBTSortedPage(page, headerPage.get_keyType());      
	...
	QuadBTIndexPage  currentIndexPage=new QuadBTIndexPage(page, 
	...
	currentIndexPage= new  QuadBTIndexPage(pinPage(currentPageId),
					   headerPage.get_keyType() );
	...
	if (currentIndexPage.available_space() >=
	    QuadBT.getKeyDataLength( upEntry.key, NodeType.INDEX))
	  {
	...
	for ( tmpEntry= currentIndexPage.getFirst( delQid);
	      tmpEntry!=null;tmpEntry= currentIndexPage.getFirst( delQid))  
	  {
	    newIndexPage.insertKey( tmpEntry.key, 
				    ((IndexData)tmpEntry.data).getData());
	    currentIndexPage.deleteSortedRecord(delQid);
	  }
	...
	QID firstQid=new QID();
	KeyDataEntry undoEntry=null;
	for (tmpEntry = newIndexPage.getFirst(firstQid);
	     (currentIndexPage.available_space() >
	      newIndexPage.available_space());
	     tmpEntry=newIndexPage.getFirst(firstQid))
	  {
	    ...
	    currentIndexPage.insertKey( tmpEntry.key, 
					((IndexData)tmpEntry.data).getData());
	    newIndexPage.deleteSortedRecord(firstQid);
          }
	...
	if (QuadBT.keyCompare( upEntry.key, tmpEntry.key) >=0 )
	  {
	  ...
	  tmpEntry =
	    QuadBT.getEntryFromBytes(currentIndexPage.getpage(), 
	  ...
	  // fill upEntry
	  upEntry= newIndexPage.getFirst(delQid);
	  ...
	  newIndexPage.deleteSortedRecord(delQid);
	...
	{
	  QuadBTLeafPage currentLeafPage = 
	    new QuadBTLeafPage(page, headerPage.get_keyType() );
	  ...
	  if (currentLeafPage.available_space() >=
	      QuadBT.getKeyDataLength(key, NodeType.LEAF) )
	    {
	      // no split has occurred
	  ...
	  if (rightPageId.pid != INVALID_PAGE)
	    {
	      QuadBTLeafPage rightPage;
	      rightPage=new QuadBTLeafPage(rightPageId, headerPage.get_keyType());
	  ...
	  for (tmpEntry = currentLeafPage.getFirst(firstQid);
	       tmpEntry != null;
	       tmpEntry = currentLeafPage.getFirst(firstQid))
	    {
	      
	      newLeafPage.insertRecord( tmpEntry.key,
					((QuadLeafData)(tmpEntry.data)).getData());
	      currentLeafPage.deleteSortedRecord(firstQid);
	  ...
	  KeyDataEntry undoEntry=null; 
	  for (tmpEntry = newLeafPage.getFirst(firstQid);
	       newLeafPage.available_space() < 
		 currentLeafPage.available_space(); 
               tmpEntry=newLeafPage.getFirst(firstQid)   )
	    {	   
	      undoEntry=tmpEntry;
	      currentLeafPage.insertRecord( tmpEntry.key,
					    ((QuadLeafData)tmpEntry.data).getData());
	      newLeafPage.deleteSortedRecord(firstQid);		
	    }
	  
	  if (QuadBT.keyCompare(key, undoEntry.key ) <  0) {
	    //undo the final record
	    if ( currentLeafPage.available_space() < 
		 newLeafPage.available_space()) {
	      newLeafPage.insertRecord( undoEntry.key, 
					((QuadLeafData)undoEntry.data).getData());
  ...
  /* 
   * findRunStart.
   * Status QuadBTreeFile::findRunStart (const void   lo_key,
   *                                QID          *pstartqid)
   *
  ...
  QuadBTLeafPage findRunStart (KeyClass lo_key, 
			   QID startqid)
    throws IOException, 
	   IteratorException,  
	   KeyNotMatchException,
	   ...
	   UnpinPageException
    {
      QuadBTLeafPage  pageLeaf;
      QuadBTIndexPage pageIndex;
      Page page;
      QuadBTSortedPage  sortPage;
      PageId pageno;
      ...
      page= pinPage(pageno);
      sortPage=new QuadBTSortedPage(page, headerPage.get_keyType());
	...
	if ( trace!=null )
      ...
      pageLeaf = new QuadBTLeafPage(page, headerPage.get_keyType() );
	...
	pageno = nextpageno; 
	pageLeaf=  new QuadBTLeafPage( pinPage(pageno), headerPage.get_keyType());    
	curEntry=pageLeaf.getFirst(startqid);
      }
	...
	// scan will unpin it when done
      ...
      while (QuadBT.keyCompare(curEntry.key, lo_key) < 0) {
  ...
  /*
   *  Status QuadBTreeFile::NaiveDelete (const void *key, const QID qid) 
   * 
  ...
  private boolean NaiveDelete ( KeyClass key, QID qid)
      ...
      if ( trace!=null )
	{
      ...
      leafPage=findRunStart(key, curQid);  // find first page,rid of key
	  ...
	  leafPage=new QuadBTLeafPage(pinPage(nextpage), 
				  headerPage.get_keyType() );
	  entry=leafPage.getFirst(new QID());
	}
	
	if ( QuadBT.keyCompare(key, entry.key) > 0 )
	  break;
	...
	leafPage=new QuadBTLeafPage(pinPage(nextpage), headerPage.get_keyType());
  ...
  private KeyClass _Delete ( KeyClass key,
      ...
      if ( trace!=null )
	{
      ...
      if (sortPage.getType()==NodeType.LEAF ) {
        ...
        PageId nextpage;
        QuadBTLeafPage leafPage;
        leafPage=new QuadBTLeafPage(page, headerPage.get_keyType());        
	
	
	KeyClass deletedKey=key;
	tmpEntry=leafPage.getFirst(curQid);
	
	QID delQid;    
	// for all records with key equal to 'key', delete it if its lid = 'lid'
	while((tmpEntry!=null) && (QuadBT.keyCompare(key,tmpEntry.key)>=0)) { 
	  ...
	  if ( leafPage.delEntry(new KeyDataEntry(key, qid)) ) {
		...
		return null;
	      ...
	      // get a sibling
	      QuadBTIndexPage  parentPage;
	      parentPage =
		new QuadBTIndexPage(pinPage(parentPageId), 
				headerPage.get_keyType());
	      
	      PageId siblingPageId=new PageId();
	      QuadBTLeafPage siblingPage;
	      int direction;
		...
		return null;
              ...
              siblingPage=new QuadBTLeafPage(pinPage(siblingPageId), 
					 headerPage.get_keyType());
		...
		if (direction==-1)
		  oldChildEntry = leafPage.getFirst(curQid);
		// get a copy
		else {
		  oldChildEntry=siblingPage.getFirst(curQid);
		}
		
		// merge the two children
		QuadBTLeafPage leftChild, rightChild;
		if (direction==-1) {
		...
		QID firstQid=new QID(), insertQid;
		for (tmpEntry= rightChild.getFirst(firstQid);
		     tmpEntry != null;
		     tmpEntry=rightChild.getFirst(firstQid)) {
		  leftChild.insertRecord(tmpEntry);
		  rightChild.deleteSortedRecord(firstQid);
		}
		
		// adjust chain
		leftChild.setNextPage(rightChild.getNextPage());
		if ( rightChild.getNextPage().pid != INVALID_PAGE) {
		  QuadBTLeafPage nextLeafPage=new QuadBTLeafPage(
	...
	indexPage=new QuadBTIndexPage(pinPage(currentPageId), headerPage.get_keyType());
	...
	PageId dummyPageId; 
	KeyClass deletedKey = key;
	QID curQid=indexPage.deleteKey(oldChildKey);
	
	if (indexPage.getCurPage().pid == headerPage.get_rootId().pid) {
	  // the index page is the root
	  if (indexPage.numberOfRecords() == 0) {
	    QuadBTSortedPage childPage;
	    childPage=new QuadBTSortedPage(indexPage.getPrevPage(),
				       headerPage.get_keyType()); 
	...
	if ((4 /*sizeof slot*/ +indexPage.available_space()) <= 
	  ...
	  // get a sibling
	  QuadBTIndexPage  parentPage;
	  parentPage=new QuadBTIndexPage(pinPage(parentPageId), 
				     headerPage.get_keyType()); 
	  
	  PageId siblingPageId=new PageId();
	  QuadBTIndexPage siblingPage;
	  int direction;
	  ...
	  siblingPage=new QuadBTIndexPage( pinPage(siblingPageId), 
				       headerPage.get_keyType());
	  
	  int pushKeySize=0;
	  if (direction==1) {
	    pushKeySize=QuadBT.getKeyLength
	  ...
	  if (siblingPage.redistribute(indexPage,parentPage, 
	    ...
	    if (direction==-1) {
	      oldChildEntry=indexPage.getFirst(curQid).key;
	    }
	    else {
	      oldChildEntry= siblingPage.getFirst(curQid).key;                    
	    }
	    
	    // merge the two children
	    QuadBTIndexPage leftChild, rightChild;
	    if (direction==-1) {
	      leftChild = siblingPage;
	    ...
	    // move all entries from rightChild to leftChild
	    for (KeyDataEntry tmpEntry=rightChild.getFirst(firstQid);
		 tmpEntry != null;
		 tmpEntry=rightChild.getFirst(firstQid) ) {
   ...
   *@exception UnpinPageException error when unpin a page
   */
  public QuadBTFileScan new_scan(KeyClass lo_key, KeyClass hi_key)
    throws IOException,  
	   KeyNotMatchException, 
	   IteratorException, 
	   ConstructPageException, 
    ...
    {
      QuadBTFileScan scan = new QuadBTFileScan();
      if ( headerPage.get_rootId().pid==INVALID_PAGE) {
	scan.leafPage=null;
	return scan;
      }
      ...
      scan.leafPage=findRunStart( lo_key, scan.curQid);
      return scan;
    }
  ...
  void trace_children(PageId id)
	...
	KeyClass key;
	KeyDataEntry entry;
	sortedPage=new QuadBTSortedPage( pinPage( id), headerPage.get_keyType());
	
	
	// Now print all the child nodes of the page.  
	if( sortedPage.getType()==NodeType.INDEX) {
	  QuadBTIndexPage indexPage=new QuadBTIndexPage(sortedPage,headerPage.get_keyType()); 
	  trace.writeBytes("INDEX CHILDREN " + id + " nodes" + lineSep);
	  trace.writeBytes( " " + indexPage.getPrevPage());
	  for ( entry = indexPage.getFirst( metaQid );
		entry != null;
		entry = indexPage.getNext( metaQid ) )
	    {
	      trace.writeBytes( "   " + ((IndexData)entry.data).getData());
	    }
	}
	else if( sortedPage.getType()==NodeType.LEAF) {
	  QuadBTLeafPage leafPage=new QuadBTLeafPage(sortedPage,headerPage.get_keyType()); 
	  trace.writeBytes("LEAF CHILDREN " + id + " nodes" + lineSep);
	  for ( entry = leafPage.getFirst( metaQid );
		entry != null;
		entry = leafPage.getNext( metaQid ) )
	    {
