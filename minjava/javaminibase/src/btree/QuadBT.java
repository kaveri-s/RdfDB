 ...
 *
 */
public class QuadBT  implements GlobalConst{
           ...
           ConvertException
    {
      ...
      try{
        ...
        if( entry.data instanceof IndexData )
          n+=4;
        else if (entry.data instanceof QuadLeafData )      
          n+=8;
        ...
        if ( entry.data instanceof IndexData ) {
          ...
          Convert.setIntValue( ((QuadLeafData)entry.data).getData().slotNo,
                               m, data);
          Convert.setIntValue( ((QuadLeafData)entry.data).getData().pageNo.pid,
                               m+4, data);
  ...
  public static void printPage(PageId pageno, int keyType)
    ...
    {
      QuadBTSortedPage sortedPage=new QuadBTSortedPage(pageno, keyType);
      int i;
      i=0;
      if ( sortedPage.getType()==NodeType.INDEX ) {
        QuadBTIndexPage indexPage=new QuadBTIndexPage((Page)sortedPage, keyType);
        System.out.println("");
        ...
        QuadBTLeafPage leafPage=new QuadBTLeafPage((Page)sortedPage, keyType);
        System.out.println("");
        ...
        for(KeyDataEntry entry=leafPage.getFirst(qid); entry!=null; 
          ...
          if( keyType==AttrType.attrInteger) 
            System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
                               (IntegerKey)entry.key+ ",  "+(QuadLeafData)entry.data+ " )");
          if( keyType==AttrType.attrString) 
            System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
                               (StringKey)entry.key + ",  "+(QuadLeafData)entry.data); 
  ...
  private static void _printTree(PageId currentPageId, String prefix, int i, 
           ...
           ReplacerException 
    {
      ...
      QuadBTSortedPage sortedPage=new QuadBTSortedPage(currentPageId, keyType);
      prefix=prefix+"       ";
      i++;
      if( sortedPage.getType()==NodeType.INDEX) {  
        QuadBTIndexPage indexPage=new QuadBTIndexPage((Page)sortedPage, keyType);
   ...
   *@exception ReplacerException  error from lower layer
   */
  public static void printAllLeafPages(QuadBTreeHeaderPage header) 
    throws IOException, 
           ConstructPageException, 
           IteratorException,
           HashEntryNotFoundException,
