 ...
 */
public class QuadBTIndexPage extends QuadBTSortedPage{
 

  /** pin the page with pageno, and get the corresponding QuadBTIndexPage,
   ...
   */
  public QuadBTIndexPage(PageId pageno, int keyType) 
    throws IOException, 
  ...
  /**associate the QuadBTIndexPage instance with the Page instance,
   * also it sets the type of node to be NodeType.INDEX.
   ...
   *@exception ConstructPageException error when BTIndexpage constructor
   */
  public QuadBTIndexPage(Page page, int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(page, keyType);
   ...
   *@exception ConstructPageException error when BTIndexpage constructor
   */
  public QuadBTIndexPage( int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(keyType);
  ...
  QID deleteKey(KeyClass key) 
	...
	while ( QuadBT.keyCompare(key, entry.key) > 0) {
	  entry = getNext(qid );
	  if (entry == null)
            break;
	}
	...
	else if (QuadBT.keyCompare(key, entry.key) != 0)
  ...
  PageId getPageNoByKey(KeyClass key) 
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  entry= QuadBT.getEntryFromBytes( getpage(),getSlotOffset(i), 
  ...
  public KeyDataEntry getFirst(QID qid) 
	...
	entry= QuadBT.getEntryFromBytes( getpage(),getSlotOffset(0), 
				    getSlotLength(0),
  ...
  int  getSibling(KeyClass key, PageId pageNo)
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  ...
	  if (QuadBT.keyCompare(key, entry.key)>=0) {
	    if (i != 0) { 
              entry=QuadBT.getEntryFromBytes(getpage(), getSlotOffset(i-1),
  ...
  boolean adjustKey(KeyClass newKey, KeyClass oldKey) 
	...
	if (qid==null) throw new IndexFullDeleteException(null, "Qid is null");
   ...
   */
  boolean redistribute(QuadBTIndexPage indexPage, QuadBTIndexPage parentIndexPage,
	...
	if (direction==-1) { // 'this' is the left sibling of indexPage
	  if (( getSlotLength(getSlotCnt()-1) + available_space()) > 
	      ( (MAX_SPACE-DPFIXED)/2) ) {
            ...
            firstEntry=indexPage.getFirst(dummyQid);
	  ...
	  if ( (getSlotLength(0) + available_space()) > ((MAX_SPACE-DPFIXED)/2) ) {
            ...
            firstEntry= QuadBT.getEntryFromBytes( getpage(),
					     getSlotOffset(0),
					     getSlotLength(0), keyType, NodeType.INDEX);
