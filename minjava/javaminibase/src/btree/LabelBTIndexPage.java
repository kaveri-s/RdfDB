...
/**
 * A LabelBTIndexPage is an index page on a label B+ tree.  It holds abstract 
 ...
 */
public class LabelBTIndexPage extends LabelBTSortedPage{
 

  /** pin the page with pageno, and get the corresponding LabelBTIndexPage,
   ...
   */
  public LabelBTIndexPage(PageId pageno, int keyType) 
    throws IOException, 
  ...
  /**associate the LabelBTIndexPage instance with the Page instance,
   * also it sets the type of node to be NodeType.INDEX.
   ...
   *@exception ConstructPageException error when BTIndexpage constructor
   */
  public LabelBTIndexPage(Page page, int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(page, keyType);
   ...
   *@exception ConstructPageException error when BTIndexpage constructor
   */
  public LabelBTIndexPage( int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(keyType);
   ...
   */
   public LID insertKey(KeyClass key, PageId pageNo) 
  ...
  LID deleteKey(KeyClass key) 
	...
	while ( LabelBT.keyCompare(key, entry.key) > 0) {
	  entry = getNext(lid );
	  if (entry == null)
            break;
	}
	
	if (entry == null) lid.slotNo--;
	else if (LabelBT.keyCompare(key, entry.key) != 0)
	  lid.slotNo--; // we want to delete the previous key
	
	deleteSortedRecord(lid);
	return lid;
      }
      catch (Exception e) {
  ...
  PageId getPageNoByKey(KeyClass key) 
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  entry= LabelBT.getEntryFromBytes( getpage(),getSlotOffset(i), 
   ...
   *@param lid It will be modified and the first rid in the index page
  ...
  public KeyDataEntry getFirst(LID lid) 
	...
	entry= LabelBT.getEntryFromBytes( getpage(),getSlotOffset(0), 
				    getSlotLength(0),
   ...
   *@param lid It will be modified and next rid will be passed out by itself.
  ...
  int  getSibling(KeyClass key, PageId pageNo)
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  ...
	  if (LabelBT.keyCompare(key, entry.key)>=0) {
	    if (i != 0) { 
              entry=LabelBT.getEntryFromBytes(getpage(), getSlotOffset(i-1),
  ...
  boolean adjustKey(KeyClass newKey, KeyClass oldKey) 
	...
	LID lid=deleteKey( entry.key );
	if (lid==null) throw new IndexFullDeleteException(null, "Lid is null");
	
	lid=insertKey( newKey, ((IndexData)entry.data).getData());        
	if (lid==null) throw new IndexFullDeleteException(null, "Lid is null");
   ...
   */
  boolean redistribute(LabelBTIndexPage indexPage, LabelBTIndexPage parentIndexPage,
	...
	if (direction==-1) { // 'this' is the left sibling of indexPage
	  if (( getSlotLength(getSlotCnt()-1) + available_space()) > 
	      ( (MAX_SPACE-DPFIXED)/2) ) {
            ...
            firstEntry=indexPage.getFirst(dummyLid);
	  ...
	  if ( (getSlotLength(0) + available_space()) > ((MAX_SPACE-DPFIXED)/2) ) {
            ...
            firstEntry= LabelBT.getEntryFromBytes( getpage(),
					     getSlotOffset(0),
					     getSlotLength(0), keyType, NodeType.INDEX);
