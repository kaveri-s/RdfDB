 ...
 *
 */
public class LabelBT  implements GlobalConst{
  ...
  protected final static 
	...
	if ( nodeType==NodeType.INDEX ) {
	  ...
	  n=8;
	  LID lid=new LID();
	  lid.slotNo =   Convert.getIntValue(offset+length-8, from);
	  lid.pageNo =new PageId();
	  lid.pageNo.pid= Convert.getIntValue(offset+length-4, from); 
  ...
  protected final static byte[] getBytesFromEntry( KeyDataEntry entry ) 
	   ...
	   ConvertException
    {
	  ...
	  n+=4;
        else if (entry.data instanceof LabelLeafData )      
	  n+=8;
	  ...
	  Convert.setIntValue( ((LabelLeafData)entry.data).getData().slotNo,
			       m, data);
	  Convert.setIntValue( ((LabelLeafData)entry.data).getData().pageNo.pid,
			       m+4, data);
  ...
  public static void printPage(PageId pageno, int keyType)
    ...
    {
      LabelBTSortedPage sortedPage=new LabelBTSortedPage(pageno, keyType);
      int i;
      i=0;
      if ( sortedPage.getType()==NodeType.INDEX ) {
        LabelBTIndexPage indexPage=new LabelBTIndexPage((Page)sortedPage, keyType);
        System.out.println("");
        ...
        for(KeyDataEntry entry=indexPage.getFirst(lid); entry!=null; 
	    entry=indexPage.getNext(lid)){
	  if( keyType==AttrType.attrInteger) 
	  ...
	  i++;    
        ...
        LabelBTLeafPage leafPage=new LabelBTLeafPage((Page)sortedPage, keyType);
        System.out.println("");
        ...
        for(KeyDataEntry entry=leafPage.getFirst(lid); entry!=null; 
	    entry=leafPage.getNext(lid)){
	  if( keyType==AttrType.attrInteger) 
	    System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
			       (IntegerKey)entry.key+ ",  "+(LabelLeafData)entry.data+ " )");
	  if( keyType==AttrType.attrString) 
	    System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
			       (StringKey)entry.key + ",  "+(LabelLeafData)entry.data); 
  ...
  private static void _printTree(PageId currentPageId, String prefix, int i, 
	   ...
	   ReplacerException 
    {
      ...
      LabelBTSortedPage sortedPage=new LabelBTSortedPage(currentPageId, keyType);
      prefix=prefix+"       ";
      i++;
      if( sortedPage.getType()==NodeType.INDEX) {  
	LabelBTIndexPage indexPage=new LabelBTIndexPage((Page)sortedPage, keyType);
   ...
   *@exception ReplacerException  error from lower layer
   */
  public static void printAllLeafPages(LabelBTreeHeaderPage header) 
    throws IOException, 
	   ConstructPageException, 
	   IteratorException,
	   HashEntryNotFoundException,
